...
export const fetchRelayConfig = async (): Promise<{
  ...
  const resp = await axiosInstance.get(API_CONFIG.CONTROLLER.GET_RELAY_CONFIG);
...
export const updateRelayConfig = async (config: { relays: RelayConfig[] }) => {
  const resp = await axiosInstance.post(API_CONFIG.CONTROLLER.SET_RELAY_CONFIG, config);
  return resp.data as { status: string; message?: string };
};

export const fetchThresholds = async (): Promise<Thresholds> => {
  const resp = await axiosInstance.get(API_CONFIG.CONTROLLER.GET_THRESHOLDS);
  return resp.data as Thresholds;
};

export const setThresholds = async (thresholds: Thresholds) => {
  const resp = await axiosInstance.post(API_CONFIG.CONTROLLER.SET_THRESHOLDS, thresholds);
  return resp.data as { status: string; message?: string };
};

export const getDeviceStates = async (): Promise<DeviceStates> => {
  const resp = await axiosInstance.get(API_CONFIG.CONTROLLER.DATA);
...
export const saveConfig = async (config: {
  ...
  const resp = await axiosInstance.post(API_CONFIG.CONTROLLER.SAVE_CONFIG, config);
  return resp.data as { status: string; message?: string };
};
...
export const fetchSensorParameters = async (): Promise<SensorParameterOption[]> => {
  const resp = await axiosInstance.get(API_CONFIG.CONTROLLER.SENSORS);
